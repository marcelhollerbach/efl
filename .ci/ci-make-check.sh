...
if [ "$DISTRO" != "" ] ; then
  for tries in 1 2 3 ; do
    (docker exec --env EINA_LOG_BACKTRACE="0" --env MAKEFLAGS="-j5 -rR" --env EIO_MONITOR_POLL=1 $(cat $HOME/cid) dbus-uuidgen > /etc/machine-id && make -j2 -C src/ check-TESTS) && break
