...
void
_node_unrealize(Efl_Ui_Tree_View_Layouter_Data *pd, Efl_Ui_Tree_View_Seg_Array_Node *node)
{
   Efl_Ui_Tree_View_Layouter_Callback_Data *cb_data;
   ...
   for (i = 0; i != node->length; ++i)
     {
        ...
        if (item->layout)
          {
             cb_data = evas_object_event_callback_del(item->layout, EVAS_CALLBACK_CHANGED_SIZE_HINTS, _on_item_size_hint_change);
             free(cb_data);
          }
            ...
            if ((item->min.w == 0) && (item->min.h == 0))
              {
                 ...
                 if (min.w && min.h)
                   {
                      _item_min_calc(pd, item, min, node, segarray);
                      efl_ui_tree_view_model_unrealize(pd->modeler, item);
                   }
                 else
                   {
                      Efl_Ui_Tree_View_Layouter_Callback_Data *cb_data = calloc(1, sizeof(Efl_Ui_Tree_View_Layouter_Callback_Data));
                      cb_data->pd = pd;
                      cb_data->item = item;
                      evas_object_event_callback_add(item->layout, EVAS_CALLBACK_CHANGED_SIZE_HINTS, _on_item_size_hint_change, cb_data);
                   }
...
_efl_ui_tree_view_layouter_efl_object_constructor(Eo *obj EINA_UNUSED, Efl_Ui_Tree_View_Layouter_Data *pd)
{
